...
function updateNewsHTML() {
  ...
  <div style="background:var(--bg3);border-left:3px solid var(--g);border-radius:8px;padding:10px 12px;margin:6px 0;font-size:13px;color:var(--t2);line-height:1.6">⚡ <b style="color:var(--t)">이번엔</b> 앱 아이콘 테두리를 완전히 없앴어요.</div>

  ${S('📣 이번 업데이트')}
  ${li('🐩 <b>앱 아이콘 테두리 완전 제거</b> — 지난 두 번의 시도에도 희미하게 남아있던 테두리를 이번엔 강아지·글자만 오려내 새로 그린 매끄러운 배경에 합성하는 방식으로 바꿔서 완전히 없앴어요(홈 화면에 이미 추가했다면 삭제 후 다시 추가해야 바뀐 아이콘이 보여요).')}
